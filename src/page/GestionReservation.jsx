...
const GestionReservation = () => {
  ...
  const customizationsOptions = ['Logo personnalisé', 'Couleurs spécifiques', 'Interface adaptée', 'Aucune'];

  // Formules avec prix par défaut
  const formules = {
    Basique: { prix: 99, features: ['Support 9h-18h', 'Formation en ligne'] },
    Standard: { prix: 199, features: ['Support 24/7', 'API avancée', 'Accès prioritaire'] },
    Premium: { prix: 299, features: ['Support 24/7', 'API avancée', 'Rapports analytiques', 'Intégration CRM'] }
  };
  ...
  // Formater les données de tableau (corrige le problème d'affichage)
    ...
    <div className="containergr">
      ...
      <div className="search-filter-container bg-white p-4 rounded-lg shadow-sm mb-6">
        ...
        <div className="filter-buttons flex flex-wrap gap-2 mt-4">
          ...
          <button
            className={`btnre btn-outline ${filter === 'premium' ? 'active bg-purple-600 text-white' : ''} flex items-center`}
          ...
          <button
            className={`btnre btn-outline ${filter === 'standard' ? 'active bg-blue-600 text-white' : ''} flex items-center`}
          ...
          <button
            className={`btnre btn-outline ${filter === 'basique' ? 'active bg-gray-600 text-white' : ''} flex items-center`}
            onClick={() => setFilter('basique')}
          >
                  ...
                  <tr key={reservation.id_reservation} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`badge ${getBadgeClass(reservation.formule)} inline-flex items-center`}>
                        {reservation.formule === 'Premium' && <BadgeCheck size={14} className="mr-1" />}
                        {reservation.formule === 'Standard' && <BadgePercent size={14} className="mr-1" />}
                        {reservation.formule === 'Basique' && <Settings size={14} className="mr-1" />}
                  ...
                  <div className="invoice-section bg-gray-50 p-4 rounded-lg flex-1">
                    ...
                    {isEditing ? (
                      ...
                      <>
                        ...
                        <p><strong className="text-gray-700">Email:</strong> {currentReservation.email || '-'}</p>
                      </>
                    )}
                  </div>
                  <div className="invoice-section bg-gray-50 p-4 rounded-lg flex-1">
